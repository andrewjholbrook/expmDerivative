...
gg <- ggplot(df,aes(x=t,y=Error,color=Dimension)) +
  ...
  ggtitle("Diagonal directional derivative vs 1st order approx.") +
  theme_bw()
gg
...
#   ylab(" ") +
#   coord_cartesian(ylim=c(0,100)) +
#   ggtitle("Off/on-diagonal directional derivative vs 1st order approx.") +
#   theme_bw()
# gg3

library(ggpubr)
ggsave(ggarrange(gg,gg2,
                 ncol=2,nrow=1,
                 common.legend = TRUE,
                 legend="right"),
