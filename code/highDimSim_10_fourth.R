...
hmc <- function(t,S,input,output,method,stepSize=0.01,L=20,maxIts=1000,
  ...
  lg_Q <- log(Q_init)
  diag(lg_Q) <- 0
  cat(lg_Q, "\n", file="data/highDimSim_10_fourth.txt",append=TRUE)
...
#plot(density(sim_bridge(n=10000,tau=1,alpha=0.5)))
#plot(density(rexp(10000)*(-1)^(runif(10000)>0.5)))
...
maxIts <- 1000000
N <- 300
entries <- sim_bridge(n=S^2,tau=1,alpha=0.25) 
