...
\citet{najfeld1995derivatives} describe the relationship between the $ij$th directional derivative $\nabla_{\JJ_{ij}} \exp(t\QQ)=:\partial (\exp t\QQ )/ \partial q_{ij}$ and the gradient of the $ij$th entry of $\exp(t\QQ)$ with respect to $\QQ$
\begin{align*}
...
\begin{align*}
	...
	&= \sum_{klmn}  \frac{\partial \exp(t\QQ)_{ij}}{\partial q_{kl}} \Big(\delta_{km} \big( \delta_{kl} \left(\delta_{mn}-1\right) + \delta_{ln}\left(1-\delta_{kl}\right) \big) \Big) \frac{\partial \lambda_{mn}}{\partial \theta} \\
	&= \sum_{klmn} \left[ \exp(t\QQ^T)  \sum_{x=0}^\infty \frac{t^{x+1}}{(x+1)!} \{\JJ_{ij},(\QQ^T)^x\}\right]_{kl} \Big(\delta_{km} \big( \delta_{kl} \left(\delta_{mn}-1\right) + \delta_{ln}\left(1-\delta_{kl}\right) \big) \Big) \frac{\partial \lambda_{mn}}{\partial \theta}
\end{align*}
