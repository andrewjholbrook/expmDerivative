...
\end{align*}
In particular, we are interested in the derivative with respect to the $S(S-1)$ free parameters $q_{ij}, i\neq j$, and these collectively correspond to the derivative in the direction of the hollow matrix
\begin{align*}
\JJ - \II =: \begin{bmatrix}
	1 & 1 &\cdots & 1 \\
1&	1 &   & \\
...
lead to fact
\begin{align*}
 \exp(t\QQ)	\{\JJ - \II, \QQ^1\} &=  \exp(t\QQ)	\left( \left(\JJ - \II\right) \QQ - \QQ \left(\JJ - \II \right)   \right) \\
&= \JJ \QQ \, .
\end{align*} 
Further, if we assume $ \exp(t\QQ)	\{\JJ - \II, \QQ^{k}\} = \JJ\QQ^{k} $ for $k>0$, we have
\begin{align*}
 \exp(t\QQ)	\{\JJ - \II, \QQ^{k+1}\} &= \exp(t\QQ)  \left( 	\{\JJ - \II, \QQ^{k}\} \QQ - \QQ 	\{\JJ - \II, \QQ^{k}\} \right)  \\
 &= \JJ\QQ^{k} \QQ - \exp(t\QQ) \QQ 	\{\JJ - \II, \QQ^{k}\} \\
 &= \JJ\QQ^{k+1}  - \QQ \exp(t\QQ)  \{\JJ - \II, \QQ^{k}\} \\ 
 &= \JJ\QQ^{k+1} - \QQ \JJ \QQ^k \\
 &= \JJ \QQ^{k+1} \, .
\end{align*}
By an inductive argument, we may therefore write the following special case for $\eqref{eq:deriv}$
\begin{align*}
	\nabla_{\JJ-\II} \exp(t \QQ)  &=t \exp(t\QQ)  (\JJ-\II) + \sum_{k=1}^\infty \frac{t^{k+1}}{(k+1)!} \JJ \QQ^k \\
	&= t (\JJ - \exp(t\QQ)) + \JJ \sum_{k=1}^\infty \frac{t^{k+1}}{(k+1)!}  \QQ^k \, .
\end{align*}
We therefore define the 1st-order approximate derivative
\begin{align*}
		\widetilde{\nabla}_{\JJ-\II} \exp(t \QQ)  :=  t (\JJ - \exp(t\QQ)) 
\end{align*}
and wish to bound the residual term
\begin{align*}
	\RR := \JJ \sum_{k=1}^\infty \frac{t^{k+1}}{(k+1)!}  \QQ^k \, .
\end{align*}
